...
public class Sell extends JFrame implements ActionListener {
	...
	Sell(){
		...
		try {
			...
			String query = "select brand , model, count(*) as model_count , status from orders group by model , status";
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource() == submit) {
			...
			try {
				...
				String query1 = "select  brand , model, count(*) as model_count , status from orders where year = '"+year.getText()+"'group by model ,status";
				String query2 = "select  brand , model, count(*) as model_count , status from orders where status = 'Delivered' and year = '"+year.getText()+"'group by model ,status";
